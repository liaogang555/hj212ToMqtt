...
public class MqttClientImpl implements MqttClient {
    ...
    private void scheduleConnectIfRequired(String host, int port, boolean reconnect) {
        if (clientConfig.isReconnect() && !disconnected) {
            log.info("going to reconnect host {}, port {}.", host, port);
            if (reconnect) {
                this.reconnect = true;
            }
    ...
    @Override
    public Future<Void> on(String topic) {
        return on(topic, defaultHandler);
    }
...
//            ch.pipeline().addLast("mqttPingHandler", new MqttPingHandler(MqttClientImpl.this.clientConfig.getTimeoutSeconds()));
            ch.pipeline().addLast("mqttPingHandler", new MqttPingHandler(MqttClientImpl.this.clientConfig.getHeartbeatInterval()));
